...
function Trends() {
    ...
    const three_month_data = [[3.4,"E.Coli"],[2.1,"Chlamydia"],[5.2,"HIV"]]
    const six_month_data = [[3.1,"E.Coli"],[4.3,"Chlamydia"],[3.2,"HIV"]]
    const one_year_data = [[3.1,"E.Coli"],[6.3,"Chlamydia"],[2.2,"HIV"]]

    const all_data = [[three_month_data, three_month], [six_month_data, six_month], [one_year_data, one_year]]
    ...
    const verticalRows = () => {
        return (
            all_data.map(
                (periodTrend,index) => {return(
                <div 
                key = {periodTrend[1]}
                className="border border-4 border-red-700 h-1/3">
                    
                    <h1 className = "text-center h-1/6">{indexCheck(index)} Increasing Trends</h1>
                    <h2 className = "text-center h-1/6">Increasing at least 5% in {indexCheck(index)}</h2>
